/* Intro: For some unknown reason most of our developers left the company. We need to actively hire now. In the media we've read that companies that invent and publish new technologies attract more potential candidates. We need to use this opportunity and invent and publish some npm packages. Following the new trend of functional programming in JS we decided to develop a functional utility library. This will put us on the bleading edge since we are pretty much sure no one else did anything similar. We also provided some jsdoc along with the functions, but it might sometimes be inaccurate.
Exercise: Provide proper typing for the specified functions.
Bonus: Could you please also refactor the code to reduce code duplication? You might need some excessive type casting to make it really short. */
...
/** 2 arguments passed: returns a new array which is a result of input being mapped using the specified mapper. 1 argument passed: returns a function which accepts an input and returns a new array which is a result of input being mapped using original mapper. 0 arguments passed: returns itself.
...
/** 2 arguments passed: returns a new array which is a result of input being filtered using the specified filter function. 1 argument passed: returns a function which accepts an input and returns a new array which is a result of input being filtered using original filter function. 0 arguments passed: returns itself.
...
/** 3 arguments passed: reduces input array it using the specified reducer and initial value and returns the result. 2 arguments passed: returns a function which accepts input array and reduces it using previously specified reducer and initial value and returns the result. 1 argument passed: returns a function which: when 2 arguments is passed to the subfunction, it reduces the input array using specified initial value and previously specified reducer and returns the result. when 1 argument is passed to the subfunction, it returns a function which expects the input array and reduces the specified input array using previously specified reducer and inital value. when 0 argument is passed to the subfunction, it returns itself. 0 arguments passed: returns itself.
